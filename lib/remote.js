(function() {
  ...
  Vnu = (function() {
    function Vnu(xargs, args, vnuPath, verbose, port1) {
      this.xargs = xargs != null ? xargs : {};
    ...
    Vnu.prototype["open"] = function() {
      return q.nfcall(freeport).then((function(_this) {
        return function(port) {
          var argsToPass, defer, e, error, server, stderrData;
          if (_this.port == null) {
            _this.port = port;
          }
          argsToPass = helper.genArgs(_this.xargs, true).concat("-cp", _this.vnuPath, "nu.validator.servlet.Main", _this.port.toString(10), helper.genArgs(_this.args));
          defer = q.defer();
          stderrData = [];
          try {
            server = _this.server = spawn(helper.javaBin(), argsToPass);
